...
export default function PropertyForm({ initial, onConfirm }: Props) {
  ...
  function set<K extends keyof PropertyData>(key: K, raw: string) {
    const numericKeys: (keyof PropertyData)[] = [
      ...
      'lotSize', 'hoaFees', 'propertyTax', 'walkScore', 'schoolRating',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Lot Size" value={data.lotSize} onChange={(v) => set('lotSize', v)} type="number" suffix="sqft" />
        ...
        <Field label="Walk Score (0–100)" value={data.walkScore} onChange={(v) => set('walkScore', v)} type="number" />
        <Field label="School Rating (1–10)" value={data.schoolRating} onChange={(v) => set('schoolRating', v)} type="number" />
      </div>
